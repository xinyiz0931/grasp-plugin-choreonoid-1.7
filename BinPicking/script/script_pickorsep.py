...
res_mz = pick_or_sep(img_path=crop_mz_path, h_params=cfg['gripper'], bin='mid')

if res_mz is None: 
    warning_print("Middle zone is empty! ")
    res_pz = pick_or_sep(img_path=crop_pz_path, h_params=cfg['gripper'], bin='pick')
    cls_pz, best_grasp = res_pz
    grasp_for_drawn = [[None, best_grasp[0], best_grasp[1], None, best_grasp[2], None, None]]
    crop_grasp_pz = draw_grasps(grasp_for_drawn, crop_pz_path,  cfg['gripper'], top_color=(0,255,0), top_only=True)
    ...
    cls_mz, best_grasp = res_mz
    grasp_for_drawn = [[None, best_grasp[0], best_grasp[1], None, best_grasp[2], None, None]]
    crop_grasp_mz = draw_grasps(grasp_for_drawn, crop_mz_path,  cfg['gripper'], top_color=(0,255,0), top_only=True)
    cv2.imwrite(draw_path, crop_grasp_mz)
...
# ----------------------- read motion file (tmp) ----------------------
# if found_cnoid: 
#     calib_mf_path = os.path.join(root_dir, "data/motion/calib_new_gripper.dat") 
#     plan_success = load_motionfile(calib_mf_path)
#     nxt = NxtRobot(host='[::]:15005')
#     motion_seq = get_motion()
#     num_seq = int(len(motion_seq)/21)
#     print(f"Total {num_seq} motion sequences! ")
