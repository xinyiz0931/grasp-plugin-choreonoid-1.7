...
bool PA10_Arm::IK_arm(const Vector3& p, const Matrix33& R){
	double phi=0;
	dvector q_old(nJoints);
...
bool  PA10_Arm::IK_arm(const Vector3& p, const Matrix33& Rp, double phi, const dvector& q_old){

		double eps = 1.0e-20;
		dvector q(7);
		Matrix33 Rb_0, R_30, R3_4, R7_e, R;
		...
		Matrix33 R_( (base->attitude()*Rb_0).transpose()*R*(R7_e).transpose() );
		Vector3  p_( (base->attitude()*Rb_0).transpose()*(p - base->p()) );	// 20140206 t.mizuno
		...
		Matrix33 As(  sqew(u_sw)*R_30);
		Matrix33 Bs( -sqew(u_sw)*sqew(u_sw)*R_30 );
		Matrix33 Cs(  m33(u_sw)*R_30 );
		...
		Matrix33 Aw(trans(R3_4)*trans(As)*R_);
		Matrix33 Bw(trans(R3_4)*trans(Bs)*R_);
		Matrix33 Cw(trans(R3_4)*trans(Cs)*R_);
		...
		for(int l=0; l<7; l++) arm_path->joint(l)->q() = q(l);	// 20140206 t.mizuno
